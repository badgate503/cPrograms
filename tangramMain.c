...
string gameList[GAME_NUM] = {"方块","天鹅"};
...
void initUI()
{
	...
	uiBtnList[0] = genBtn(4,1.3,1,0.5,"开始",35);
	uiBtnList[1] = genBtn(5,1.3,1,0.5,"载入",35);
	uiBtnList[2] = genBtn(6,1.3,1,0.5,"关于",35);
	uiBtnList[3] = genBtn(7,1.3,1,0.5,"退出",35);
...
void initTangrams(bool isDefault)
{
	...
	btnList[0] = genBtn(.1,0.1,1,0.5,"载入游戏",20);
	btnList[1] = genBtn(.1,1.6,1,0.5,"保存游戏",20);
	btnList[2] = genBtn(.1,3.1,0.5,0.5,"→",30);
	btnList[3] = genBtn(.1,4.1,1,0.5,"帮助",20);
	btnList[4] = genBtn(.1,5.6,1,0.5,"返回菜单",20);
	updateDisplay(0);
